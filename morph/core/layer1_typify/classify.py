...
def typify_word(text: str,
    ...
    Args:
        ...
        use_fuzzy: (DEPRECATED) Use domain_config.use_fuzzy instead.
        domain_config: DomainConfig instance (v2.1+).

    Returns:
        One of :data:`PARTICLE_TYPES`.
    """
    # Handle domain_config (v2.1+) or fallback to legacy parameters
    if domain_config is not None:
        _model_patterns = domain_config.get_model_patterns()
        _size_patterns = domain_config.get_size_patterns()
        _exclude_patterns = domain_config.get_exclude_patterns()
        _spec_terms = domain_config.spec_terms
        _spec_substrings = domain_config.spec_substrings
        _section_terms = domain_config.section_terms
        _unit_set = domain_config.unit_set
        _use_fuzzy = domain_config.use_fuzzy
        _domain_enabled = len(_spec_terms) > 0 or len(_model_patterns) > 0
    else:
        # Legacy mode (backward compatibility)
        _model_patterns = model_patterns or []
        _size_patterns = size_patterns or []
        _exclude_patterns = EXCLUDE_PATTERNS
        _spec_terms = SPEC_TERMS if domain else set()
        _spec_substrings = SPEC_SUBSTRINGS if domain else ()
        _section_terms = SECTION_TERMS if domain else set()
        _unit_set = UNIT_SET
        _use_fuzzy = use_fuzzy
        _domain_enabled = domain
    ...
    if is_bold and _domain_enabled and re.match(r'^[A-Z0-9][A-Z0-9\-]{2,}$', t):
        return 'MODEL'
    ...
    # --- Domain-specific block (skip if domain disabled) ---
    if _domain_enabled:
    ...
    # --- Domain-specific vocabulary (skip if domain disabled) ---
    if _domain_enabled:
        # 6. Section header (with fuzzy matching if enabled)
        words_in = set(tl.split())
        if words_in & _section_terms and len(t) > 8:
            return 'SECTION'

        # Fuzzy match for SECTION (typo/OCR tolerance)
        if _use_fuzzy and len(t) > 8:
            try:
                from rapidfuzz import fuzz
                for section_term in _section_terms:
        ...
        if words_in & _spec_terms:
            return 'SPEC_LABEL'
        if len(t) > 12 and any(kw in tl for kw in _spec_substrings):
            return 'SPEC_LABEL'

        # Fuzzy match for SPEC_LABEL (typo/OCR tolerance)
        if _use_fuzzy and len(t) > 5:
            try:
                from rapidfuzz import fuzz
                for spec_term in _spec_terms:
